...
/**
 * Strip a leading "Dataset:" / "Dataset -" / "Dataset –" prefix from a
 * dataset name. The cloud admin UI lets uploaders save names with a
 * literal `Dataset:` prefix; some legacy entries have it, newer entries
 * don't. Surface them uniformly on read so cards / hero / page titles
 * don't show "Dataset: Dataset: …" or read like a placeholder.
 *
 * Conservative: only strips the very first prefix, leaves the rest of
 * the string (including the substring "dataset" mid-sentence)
...
export function cleanDatasetName(
  ...
  // `–`, em-dash) + whitespace. Case-insensitive for robustness against
  // "DATASET:" / "dataset:" / "Dataset -" variants.
  return name
    .trim()
 ...
 *
 * The synthesizer pipeline injects `DATASET BEING PROCESSED.` at the
 * head of abstracts whose enrichment is in-flight. That marker is
 * meant for ops, not readers — it leaks into catalog cards + the
 * detail Overview tab and reads like a system-error message inside
 * the abstract paragraph. Strip it on render; the optional `marker`
 * return tells the caller it was present so a "Processing" badge
 * can render alongside the abstract if desired.
 *
...
export function cleanAbstract(
  ...
  // Anchor on the literal placeholder (case-insensitive). The trailing
  // period + optional whitespace is consumed so the next sentence
  // doesn't read with leading punctuation.
