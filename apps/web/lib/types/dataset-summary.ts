 ...
 *
 * Ported verbatim from `ndi-data-browser-v2/frontend/src/types/dataset-summary.ts`.
 * One-to-one mirror of `backend/services/dataset_summary_service.py::DatasetSummary`.
 * Produced by the Plan B B1 synthesizer and returned verbatim from
 ...
 * the raw cloud-record shape at `@/lib/api/datasets.DatasetRecord`; that's
...
export interface DatasetSummaryCitation {
   ...
   *  true publication year. B4's cite modal labels this explicitly as
   *  "upload year". */
  year: number | null;
}
 ...
 * accidentally erase the schema version string at build time. */
export const DatasetSummaryContract = { schemaVersion: 'summary:v1' } as const;

/**
 * Compact catalog-card projection of `DatasetSummary` (Plan B B2).
 *
 ...
 * `DatasetSummary` adds ~100KB per page. This compact projection is
 * ~400-600 bytes per row. Clients that need the full shape still hit
 * `GET /api/datasets/:id/summary`.
 */
