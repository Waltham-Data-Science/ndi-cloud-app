 ...
 *
 * Ported verbatim from `ndi-data-browser-v2/frontend/src/data/table-column-definitions.ts`
 * (Phase 6.5a of the cross-repo unification — see
 * `docs/plans/cross-repo-unification-2026-04-24.md`). No semantic changes;
 * only the import path is monorepo-relative.
 */
...
const definitions: Record<string, ColumnTooltip> = {
  ...
  element_epoch_epochStart: {
    label: 'Start',
    description:
      'Epoch start time. Objects with {devTime, globalTime} — devTime is device-local seconds, globalTime is the synced experiment clock (null for scalar-clock datasets).',
  },
