/**
 * Ported from `ndi-data-browser-v2/frontend/src/lib/safe-href.test.ts` —
 * one test added for the SSR sentinel path (relative URL with no
 * `window.location.origin`) since that's the new code path in this port.
 */
...
describe('safeHref', () => {
  ...
  describe('accepts safe schemes', () => {
    ...
    it('resolves relative paths against origin (browser context)', () => {
