...
export function DatasetOverviewCard({
  ...
  // `cleanAbstract` strips a leading "DATASET BEING PROCESSED." marker
  // (synthesizer-pipeline placeholder) and reports it via `processing`
  // so we can render a discrete badge alongside the prose instead of
      ...
      <CardHeader>
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          ...
          {/* See `cleanAbstract` — `Processing` surfaces the synthesizer
              in-flight marker as a discrete pill instead of leaking it
