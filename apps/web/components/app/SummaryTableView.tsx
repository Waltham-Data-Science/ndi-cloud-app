...
/**
 * SummaryTableView — fully-featured summary table for `/datasets/[id]/tables/[className]`.
 *
 * Ported from `ndi-data-browser-v2/frontend/src/components/tables/SummaryTableView.tsx`
 * (Phase 6.5a of the cross-repo unification — see
 * `docs/plans/cross-repo-unification-2026-04-24.md`). Three monorepo
 * adaptations vs. the v2 source:
 *
 *   1. URL-persisted state uses Next's `useSearchParams` + `useRouter`
 *      (replace, no history-stack growth) instead of react-router-dom's
 *      `setSearchParams({ replace: true })`. The contract is identical:
 *      `?tq` (filter), `?tsort` (sort), `?thide` (hidden cols).
 *   2. Imports rewritten for monorepo layout (`@/lib/api/...`,
 *      `@/components/ui/...`, `@/lib/data/...`).
 *   3. The embedded **QuickPlot card is deferred** — see the FOLLOW-UP
 *      block at the bottom. The full table view (filter, sort, column
 *      toggle, CSV/XLS/JSON export, ontology popovers, virtualization)
 *      ships now; the violin-plot card lands when `ViolinPlot` +
 *      `useDistribution` port to the monorepo.
 *
 * Original behavior:
 *
...
interface SummaryTableViewProps {
   ...
   * enable the QuickPlot card (deferred — see FOLLOW-UP at bottom). */
  datasetId?: string;
}
...
export function SummaryTableView({
  ...
  // The prefix `t` scopes these under the SummaryTableView namespace so
  // they don't collide with page-level params.
  //
  // Next's `useSearchParams` is read-only — `useRouter().replace(href)`
  // applies the new query string without growing the history stack
  // (matches react-router-dom's `setSearchParams({ replace: true })`).
   ...
   * clean on the default/empty case. `router.replace` to avoid stacking
   * history entries on every keystroke. `scroll: false` so updating the
   * filter doesn't scroll the page back to the top.
   *
   * `searchParams` is read-only in Next, so we copy into a fresh
   * `URLSearchParams`, mutate, then synthesize the new URL. */
  ...
  useEffect(() => {
    if (sorting.length === 0) {
      ...
      const s = sorting[0];
      if (s) updateParam('tsort', `${s.id}:${s.desc ? 'desc' : 'asc'}`);
    }
  }, [sorting, updateParam]);
  ...
  // React Compiler / `react-hooks/incompatible-library`: TanStack Table's
  // `useReactTable()` returns identity-changing helpers (`getRowModel`,
  // `getFilteredRowModel`, etc.) that are designed to be called directly
  // in the render path. The lint rule's "values may be passed to memoized
  // children" warning doesn't apply here — the table object is consumed
  // by `<VirtualizedTable>` immediately below, which itself is unmemoized.
  // Disabling at the call site, not globally, so other consumers of this
  // lint rule continue to flag genuine misuse.
  ...
  //
  // Uses `@e965/xlsx` — the maintained npm fork of SheetJS's `xlsx`
  // package. The original `xlsx@0.18.5` on npm has two unpatched
  // high-severity advisories (GHSA-5pgg-2g8v-p4x9 plus a prototype
  // pollution issue); SheetJS migrated to publishing via their CDN
  // tarball (data-browser does that), but this repo's sandbox blocks
  // CDN installs, so we use the same patched code via the npm fork.
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          {hiddenByAuto > 0 && (
            <button
              type="button"
              className="text-[10px] text-fg-muted hover:text-fg-secondary underline decoration-dotted"
              onClick={() => {
      ...
      {showColumnPicker && (
        <div className="flex flex-wrap gap-2 p-2 rounded-md border border-border-subtle bg-bg-muted">
...
function ColumnInfoTip({
  ...
  return (
    <>
      ...
      >
        <Info
          className="h-3 w-3 text-fg-muted hover:text-fg-secondary cursor-help"
      ...
      <FloatingPanel
        ...
        width={256}
        estimatedHeight={80}
        className="rounded-md border border-border-subtle bg-bg-surface p-2 shadow-lg text-xs text-fg-secondary"
      >
        {description}
      </FloatingPanel>
    </>
    ...
    return (
      ...
      >
        ...
        {global !== null && global !== undefined && (
          <span className="text-[10px] text-fg-muted">{String(global)}</span>
        )}
      </span>
    );
  }
