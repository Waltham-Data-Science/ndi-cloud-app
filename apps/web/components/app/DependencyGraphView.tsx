 ...
 *
 * Visual + text dependency graph for a single NDI document. Ported
 * verbatim from
 * `ndi-data-browser-v2/frontend/src/components/documents/DependencyGraph.tsx`
 * with three monorepo adapter changes:
 *
 *   1. `react-router-dom` `Link to=` → `next/link` `Link href=`.
 *   2. `'use client'` directive at top — uses `useState` + click
 *      handlers, can't be a server component.
 *   3. The source had a malformed Tailwind class
 *      `'hover:text-gray-900:text-gray-100'` (lines 153/168) that was
 *      a leftover dark-mode variant — corrected to
 *      `'hover:text-gray-900'` here. Confirmed via the source class
 *      list: this was a typo, not an intentional invalid class.
 *
 * NOT a D3 component: despite the original sequencing brief framing
 * REBUILD-9 as "first D3 import," the source uses pure
 * CSS-Flexbox + per-node Tailwind cards + `<Connector>` lines. No D3
 * dependency lands here. REBUILD-11 (QuickPlot + ViolinPlot D3 KDE)
 * is where D3 actually arrives in the monorepo.
 *
 * Renders inside the document-detail page below `<DocumentDetailView>`
 * + `<AppearsElsewhere>`. Two view modes ("visual" tree vs. "list"
 * text) toggle via the header button strip; visual is the default.
 *
 * Empty/leaf states: the component shows a compact `<DepGraphEmpty>`
 * card with an explanatory message rather than rendering nothing,
 * because users were misreading the silent-empty as a broken feature.
 */
import {
  ArrowDown,
  ArrowUp,
...
export function DependencyGraphView({
  ...
  // feature was broken when in fact root documents like `subject`
  // simply have no dependencies to walk.
  ...
  const downstreamEdges = graph.edges.filter(
...
function NodeBox({
  ...
  const content = (
    <div
      className={cn(
        ...
        isTarget
          ? 'border-brand-500 bg-brand-50 ring-2 ring-brand-200/60'
    ...
    >
      ...
      <span
        className={cn(
          ...
          isTarget ? 'text-brand-800 font-medium' : 'text-gray-600',
        )}
      >
        {label}
      </span>
  ...
  return (
    <Link
      href={`/datasets/${datasetId}/documents/${node.id}`}
...
function EdgeRow({
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      {linkTarget ? (
        <Link
          href={`/datasets/${datasetId}/documents/${linkTarget}`}
