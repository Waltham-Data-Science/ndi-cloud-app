 ...
 *
 * Distinct from `MarketingButton` in `components/marketing/Button.tsx`
 * which has a different design intent (cta/ghost/outline + larger size
 * scale for marketing pages). This is the data-browser-shape button
 * used inside `(app)/*` routes.
 *
 * Variants:
 *   primary:   teal action with CTA glow shadow
 *   secondary: white "ghost" with navy border
 *   ghost:     transparent hover tint
 *   danger:    red confirmation for destructive actions
 *
 * All colors via design tokens so button surfaces stay in lockstep with
